...
<form action="" method="post" enctype="multipart/form-data">
	@csrf
	@if($post=DB::table('post')->where('postid',$postid)->first())
	<input type="text" name="title" placeholder="cập nhập tiêu đề" class="form-control" value="{{$post->title}}">
	<input type="text" name="content" placeholder="cập nhập nội dung" class="form-control" value="{{$post->content}}">
	<img src="/image/{{$post->img}}" class="form-control img-thumbnail">
	<input type="hidden" name="img_hidden" value="{{$post->img}}">
	<input type="file" name="img" placeholder="cập ảnh tại đây" class="form-control">
